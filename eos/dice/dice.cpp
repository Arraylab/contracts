...
class EOSBetDice : public eosio::contract {
	public:
		...
		const uint32_t refund_delay_sec = 24 * 3600;
		...
		void reveal(const uint64_t bet_id, const std::string& house_seed) {
			...
			uint64_t payout = 0;
			if (activebets_itr->referral != activebets_itr->bettor && activebets_itr->referral != _self){
			...
			if (ref_reward > 0){
				ref_tx.actions.emplace_back(
					...
					std::make_tuple(
						_self,
						activebets_itr->referral,
						asset(ref_reward, EOS_SYMBOL),
						name_to_string(activebets_itr->referral) + std::string(" Bet id: ") + std::to_string(bet_id) + std::string(" -- Referral reward! Play: palydice.io")
					)
				);
			}
	...
	private:
		...
		void airdrop_tokens(const uint64_t bet_id, const uint64_t bet_amt, const account_name bettor){
			// long cur_scale = get_current_scale()
			uint64_t drop_amt = bet_amt * 10;

			const uint64_t bet_token_balance = get_token_balance( N(betdividends), symbol_type(BET_SYMBOL) );
